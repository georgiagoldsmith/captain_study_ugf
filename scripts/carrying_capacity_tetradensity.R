# Better carrying-capacity scalar than pure global Damuth's Law:
...
# using real TetraDENSITY density directly wherever a species has a match,
# and the fitted model only for the rest.
#
# Common-slope, order-specific-intercept model (ANCOVA): log10(density) ~
# log10(mass) + Order, fit on ALL ~1850 bird species with both a density
# record and a body mass (not just the 18 target species), so the small
# target orders (Bucerotiformes n=21, Gruiformes n=22, Coraciiformes n=27)
# still borrow a well-estimated common slope from the full dataset while
# getting their own order-specific baseline density level. A full
# slope*order interaction was checked but not used -- most target orders
# don't have enough species to support a stable order-specific slope on
# top of an intercept, so forcing one would overfit exactly the orders
# this model most needs to generalize for.
...
# --- STEP 1: build the training set (birds only, not our 18 -- these are
# just the species used to learn the general density~mass~order pattern) ---
...
# each order shift that same line up/down (its own intercept only) ---
...
# TetraDENSITY record, use that measured value instead of the step 3
# prediction (real data beats an estimate whenever we actually have it) ---
...
# CAPTAIN grid cell. Cells are 3km x 3km = 9 km2 (NOT 3 km2 -- area is
# side^2), so multiply density by 9 to get a literal expected headcount
# per cell rather than an arbitrary rescaled dial ---
